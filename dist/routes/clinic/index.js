...
import { Router } from "express"; // Import types
import { z } from "zod";
...
function getEffectiveClinicTag(req) {
    const user = req.user;
...
clinicRouter.get("/batches", async (req, res) => {
    ...
    if (user.role === UserRole.CLINIC) {
        if (!user.clinicTag)
            return res.status(403).json({ code: "NO_CLINIC_TAG" });
