...
async function getLatestPlanInstanceForUser(userId) {
...
function shapePlanInstance(instance) {
    return {
        id: instance.id,
...
function resolvePlanForFrontend(shaped) {
    const planJson = shaped?.planJson;
    if (!isPlainObject(planJson))
        return shaped;
    const schemaVersion = planJson.schemaVersion;
    const modulesDict = planJson.modules;
    const days = planJson.days;
    // Only resolve for schemaVersion 2 with modules + days present
    if (schemaVersion !== 2)
        return shaped;
    if (!isPlainObject(modulesDict))
        return shaped;
    if (!Array.isArray(days))
        return shaped;
    const resolvedDays = days.map((day) => {
        const moduleIds = Array.isArray(day?.moduleIds) ? day.moduleIds : [];
        const modulesResolved = moduleIds.map((id) => modulesDict[id]).filter(Boolean);
...
function resolveDayBlock(planJson, dayBlock) {
    ...
    if (!isPlainObject(modulesDict))
        return dayBlock;
    if (!isPlainObject(dayBlock))
        return dayBlock;
    const moduleIds = Array.isArray(dayBlock.moduleIds)
        ? dayBlock.moduleIds
        : [];
    const modulesResolved = moduleIds.map((id) => modulesDict[id]).filter(Boolean);
...
planRouter.post("/generate", async (req, res) => {
    ...
    const category = activation.clinicConfig?.defaultCategory ?? RecoveryPlanCategoryEnum.general_outpatient;
    const template = await prisma.recoveryPlanTemplate.findFirst({
        where: { category },
        orderBy: { version: "desc" },
    });
...
planRouter.get("/today/resolved", async (req, res) => {
    ...
    const planJson = finalInstance.planJson;
