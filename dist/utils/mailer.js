...
export async function sendVerificationEmail(params) {
  ...
  `;
    ...
    if (!resend || !from) {
        console.log(`[FRDC] (mailer not configured) Verification code for ${to}: ${params.code} (expires in ${params.expiresMinutes}m)`);
        return;
    }
    try {
        ...
        if (error) {
            ...
            console.log(`[FRDC] (fallback code) Verification code for ${to}: ${params.code} (expires in ${params.expiresMinutes}m)`);
            return;
        }
        if (data?.id) {
